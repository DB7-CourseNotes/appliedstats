...
Consider the followong plot. The red line is the estimated linear model, and the red dots are the average value of $y$ for all $x$ values that are within 2 units. For example, the point at `x = 15` is the average of the `y` values for all data where `13 <= x <= 17`.

```{r echo=FALSE}
x_avg <- cars$speed

y_avg <- sapply(x_avg, function(x0) {
  mean(cars$dist[abs(cars$speed - x0) <= 2])
})
...
points(x_avg, y_avg, col = "red", pch = 1, cex = 1.2)

legend(
  "topleft",
  legend = c("Observed data", "Linear model", "Window average"),
  col = c("grey", "darkorange", "darkorange"),
  pch = c(16, NA, 1),
  lty = c(NA, 1, NA),
  lwd = c(NA, 2, NA)
)
...
## Summation Notation for Regression
...
A prediction of $y$ for a given $x$ value can be found by plugging $x$ into 
$$
\hat{y} = \hat{\beta}_0 + \hat{\beta}_1 x.
$$
...
```{webr-r}
which(cars$speed == 8) # Returns the index of all cars with a speed of 8
...
If we square then sum both sides of the equation above^[Warning: there's an extra step involved here! Try to work through the math yourself, recalling with great care that $(a + b)^2 \ne a^2 + b^2$ in general.], we can obtain the following,
...
##  Coefficient of Determination / Variance Explained by the Regression / $R^2$

$$
\begin{aligned}
R^2 &= \frac{\text{SSReg}}{\text{SST}} \\&= \frac{\sum_{i=1}^{n}(\hat{y}_i - \bar{y})^2}{\sum_{i=1}^{n}(y_i - \bar{y})^2}\\
    &= \frac{\text{SST} - \text{SSE}}{\text{SST}} \\&= 1 - \frac{\text{SSE}}{\text{SST}}\\
...
The `summary()` command also returns a list, and we can again use `names()` to learn what about the elements of this list^[This is extraordinarily odd behaviour in R, and the more you learn about this the more weird it becomes. For now, just know that you can do it.].
...
- `epsilon = rnorm(...)` is doing the bulk of the work here.
- Each time you run this function, the results will be slightly different. More on this in the next chapter!
:::
