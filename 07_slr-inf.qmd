...
::: {.callout-note}
## Variances of Regression Estimates
...
Intuitively, if you have different data you would have gotten a different estimate. The sampling distribution describes the variability in this estimate.
:::
...
::: {.callout-note}
## Significance of Regression Test (t-test)
...
Interpretation:
- Under $H_0$: the response does **not** depend linearly on the predictor. There is **no significant linear relationship** between $x$ and $y$.
- Under $H_1$: the response depends linearly on the predictor. There **is a significant linear relationship** between $x$ and $y$.
...
::: {.callout-note}
## Formula: Prediction Interval for New Observations
...
```{webr-r fig.height = 8, fig.width = 10, message = TRUE}
